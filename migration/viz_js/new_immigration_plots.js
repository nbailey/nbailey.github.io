const mapWidth_1 = 1000;
const mapHeight_1 = 600;

const height_1 = 300;
const width_1 = 975;
const margin_1 = {top: 10, right: 10, bottom: 20, left: 40};

// const world_svg = d3.select("#niv-chart").append("svg").attr("width", mapWidth_1).attr("height", mapHeight_1);
// const niv_svg = d3.select("#niv-chart").append("svg").attr("width", width_1).attr("height", height_1);
// const sevis_svg = d3.select("#niv-chart").append("svg").attr("width", width_1).attr("height", height_1);
// const countryoforigin_svg = d3.select("#niv-chart").append("svg").attr("width", 900).attr("height", 600);
...
const countryoforigin_svg = d3.select("div#origin-select")
                  .append("div")
                  .classed("svg-container", true) //container class to make it responsive
                  .append("svg")
                  .attr("preserveAspectRatio", "xMidYMid meet")
...
const residence_colors = ["#535166", "#5E5C74", "#6A6883", "#767391",
            "#817E9F", "#8C89A7", "#9795B0", "#A3A1B9", "#AEACC1", "#BAB8CA",
            "#C5C4D3", "#D1D0DC", "#DCDBE4", "#E8E7ED"].reverse()
...
var key = elemData.key.toLowerCase().replace("\'", "").replace(" ","-");
  ...
  d3.select("#path-"+key)
    .attr("stroke", "#e7eeef")
    .attr("stroke-width_1", "5px")
    .attr("z-index", 100);

  d3.select("#legend-"+key)
    .attr("stroke", "#e7eeef")
    .attr("stroke-width_1", "3px");
  }
...
projection = d3.geoEquirectangular()
    .scale(160)
    .translate([mapWidth_1/2, mapHeight_1/2])
    .precision(0.1);
...
var map_title = world_svg.append("text")
    .attr("x", mapWidth_1/2 - 400)
    .attr("y", mapHeight_1 * 3/5 - 20)
    .attr("fill", "white");
...
world_svg.selectAll("country")
    ...
    .on("click", function (d) {
        ...
        if (country_name != null) {
          d3.select("#type-text").html("Breakdown by Visa Type and Degree Type for Students from " + country_name);
          d3.select("#where-text").html("Where Do Foreign Students from " + country_name + " Live in the U.S.?")
          setTimeout(function() {
            document.getElementById("internationalStudentData").scrollIntoView({behavior: "smooth"})
          }, 800);
        }
    })
...
d3.csv(
  ...
  ({year: +year, continent, country,a_1: +a_1, a_2: +a_2, a_3: +a_3, b_1, b_1_b2, b_2, b_1_2_bcc, b_1_2_bcv, c_1, c_1_d, c_2, c_3,
    ...
    var dateScale = d3.scaleTime()
      .domain(dateDomain)
      .range([margin_1.left, width_1-margin_1.right]);
    ...
    //nested max: looping through series to find the max of the total
      ...
      .rangeRound([height_1 - margin_1.bottom, margin_1.top]) //height_1 of svg - bottom margin_1
    ...
    nivDateAxis = g => g
        ...
        .call(d3.axisBottom(d3.scaleTime()
          ...
          .range([margin_1.left, dateScale(d3.max(niv_total_data, d=>d.Date))])
        ).ticks(width_1 / 100).tickSizeOuter(0))

    sevisDateAxis = g => g
        .attr("transform", `translate(0,${height_1 - margin_1.bottom})`)
        .call(d3.axisBottom(d3.scaleTime()
          .domain(d3.extent(sevis_total_data, d=>d.Date))
          .range([dateScale(d3.min(sevis_total_data, d=>d.Date)), width_1-margin_1.right])
        ).ticks(width_1 / 100).tickSizeOuter(0))
    ...
    var niv_legend = svg => {
      const g = svg //creating SVG for the legend
        ...
        .attr("transform", `translate(${width_1 - margin_1.right},${margin_1.top})`)
        .selectAll("g")
  ...
  d3.csv("https://gist.githubusercontent.com/jk979/466428ff2f3c27d3a877722d9cf933a2/raw/0b8e774c211e1619ef9c8a05115e0493ceaf584b/countryStateStudentNumbers.csv")
  .then(function(data){
    countryStateStudentNumbers = data;

    /////////////////////// setup /////////////////

    d3.csv("https://gist.githubusercontent.com/nbailey/95ad116b50e7b7cbe02a8e2dac626922/raw/e3d08f6b6c829885d997231988a27aa272f117cb/sevis_stem_201808")
    .then(function(data){

      var dataStem = data;
      var states = [];
      dataStem.forEach(r => states.includes(r["State"]) ? 0 : states.push(r["State"]));
      states.sort();
      ...
      for (var i in states) {
        ...
        if (state.toUpperCase() === state) {
          onlyStates.push(state);
        }
      }

      console.log(dataStem)
      var stateAbbrvMap = {};
      dataStem.forEach(r => stateAbbrvMap[r["State"]] = r["Abbrv"])
      console.log(stateAbbrvMap)

      var stateIDs = [];
      us.objects.states.geometries.forEach(d => stateIDs.push(d.id));
      stateIDs.sort();

      var stateNameMap = {};
      for (var i in onlyStates) {
        var state = onlyStates[i];
        stateNameMap[+stateIDs[i]] = state;
      }
      console.log(stateNameMap);
    ...
    function drawStateMap(){
      console.log(country)
      console.log(countryStateStudentNumbers)
      countryStateData = filter(countryStateStudentNumbers, "county_fixed", country)[0]
      console.log(Object.values(countryStateData).map(d => +d))

    /////// SVG ////////


      numColorScale = d3.scaleQuantize()
        .domain([0, Math.log(d3.max(Object.values(countryStateData).map(d => +d)))])
        .range(residence_colors);

      var g = countryoforigin_svg.append("g");
      ...
      g.selectAll("path")
          ...
          .enter().append("path")
          .attr("d", path)
          .style("fill", d => numColorScale(Math.log(+countryStateData[+d.id])))
          .style("stroke", "white")
          .on("mouseover", d => tooltip_state.style("visibility", "visible").style("background", numColorScale(Math.log(+countryStateData[+d.id]))).text(+d.id))
          .on("mousemove", d => tooltip_state
              .style("top", (d3.event.pageY-10)+"px")
              .style("left",(d3.event.pageX+10)+"px")
              .text(stateNameMap[+d.id] + ": " + countryStateData[+d.id] + " students from " + countryStateData.county_fixed))
          .on("mouseout", d => tooltip_state.style("visibility", "hidden"));
...
}); // End d3.csv NIV data call
//}); // End d3.json US Topo map call
